...
#	Description: Brook webui部署脚本 基于Toyo的脚本，增加服务端部署功能
#	Version: 1.0
#	Author: Toyo
#	Modified by Aoyouer
#=================================================
...
#还需要检测本地是否有配置文件 服务器名字 介绍 用户名 密码 监听端口
check_pid(){
	PID=$(ps -ef| grep "brook relays"| grep -v grep| grep -v ".sh"| grep -v "init.d"| grep -v "service"| awk '{print $2}')
}
check_new_ver(){
	echo -e "1. 从Github下载"
	echo -e "2. 镜像源下载(解决Github无法下载的问题)"
	read -e -p "请选择Brook下载途径:" download_way
	if test $download_way -eq 1
	then
	...
	版本列表请去这里获取：${Green_font_prefix}[ https://github.com/txthinking/brook/releases ]${Font_color_suffix}"
		...
		echo -e "${download_way}从镜像源下载,如果镜像源失效,请联系作者"
	fi
...
Download_brook(){
	...
	if [[ ${bit} == "x86_64" ]]; then
		...
		then
			...
			wget -N "https://ru.better06.xyz/brook"
		fi
	else
		if [[${download_way} == "1"}]]
		then
			...
			wget -N "https://ru.better06.xyz/brook_linux_386"
		fi
